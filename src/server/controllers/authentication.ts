...
export async function validateUser(token: string, client?: ServerClient | null) {
	try {
		...
		let loggedIn = await new Promise<UserRecord>((resolve, reject) => {
			connection.query(
				'SELECT last_logout, username FROM accounts WHERE id = ?',
				[decoded.userId],
				(err, result) => {
					if (err) reject(err);
...
export async function logout(token: string): Promise<boolean> {
	try {
		...
		let loggedOut = await new Promise((resolve, reject) => {
			connection.query(
				'UPDATE accounts SET last_logout = ? WHERE id = ?',
				[Date.now(), decoded.userId],
				(err, result) => {
					if (err) reject(err);
					// @ts-ignore
					resolve();
...
export async function getAccountInfo(token: string): Promise<object> {
	try {
		...
		return new Promise((resolve, reject) => {
			connection.query(
				'SELECT username, email, email_verified FROM accounts WHERE id = ?',
				[decoded.userId],
				(err, result) => {
					if (err) {
...
export async function changeEmail(token: string, newEmail: string): Promise<EmailChangeResponseData> {
	...
	try {
		...
		connection.query(
			'UPDATE accounts SET last_email_change = ? WHERE id = ?',
			[Date.now(), decoded.userId],
			(err) => {
				if (err) {
...
export async function changePassword(
	...
	newPassword: string,
	requireOld = true,
): Promise<PasswordChangeResponseData> {
...
export async function sendEmailVerification(token: string) {
	try {
		...
		let reset = new Promise((resolve, reject) => {
			connection.query(
				'UPDATE accounts SET email_verified = ? WHERE id = ?',
				[false, decoded.userId],
				(err, result) => {
					if (err) {
